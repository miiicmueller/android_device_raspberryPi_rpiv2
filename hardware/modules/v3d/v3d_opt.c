...
static int v3d_bin_oom_size = (1 * 1024 * 1024);
...
static DEFINE_SPINLOCK(v3d_id_lock);
...
struct semaphore v3d_jobFin;

static struct mutex v3d_mutex;
...
static struct V3dRegisterIdent __iomem *g_pV3dIdent =
		(struct V3dRegisterIdent __iomem *) IO_ADDRESS(0x20c00000);

struct V3dRegisterScratch {
	volatile unsigned int m_scratch;
};
static struct V3dRegisterScratch __iomem *g_pV3dScratch =
		(struct V3dRegisterScratch __iomem *) IO_ADDRESS(0x20c00000 + 0x10);

struct V3dRegisterCache {
	volatile unsigned int m_l2cactl;
	volatile unsigned int m_slcactl;
};
static struct V3dRegisterCache __iomem *g_pV3dCache =
		(struct V3dRegisterCache __iomem *) IO_ADDRESS(0x20c00000 + 0x20);
...
static struct V3dRegisterIntCtl __iomem *g_pV3dIntCtl =
		(struct V3dRegisterIntCtl __iomem *) IO_ADDRESS(0x20c00000 + 0x30);
...
static volatile struct V3dRegisterCle __iomem *g_pV3dCle =
		(struct V3dRegisterCle __iomem *) IO_ADDRESS(0x20c00000 + 0x100);
...
static struct V3dRegisterBinning __iomem *g_pV3dBinning =
		(struct V3dRegisterBinning __iomem *) IO_ADDRESS(0x20c00000 + 0x300);
...
static struct V3dRegisterSched __iomem *g_pV3dSched =
		(struct V3dRegisterSched __iomem *) IO_ADDRESS(0x20c00000 + 0x410);
...
static struct V3dRegisterUserProgramReq __iomem *g_pV3dUpr =
		(struct V3dRegisterUserProgramReq __iomem *) IO_ADDRESS(
				0x20c00000 + 0x430);
...
static struct V3dRegisterVpm __iomem *g_pV3dVpm =
		(struct V3dRegisterVpm __iomem *) IO_ADDRESS(0x20c00000 + 0x500);

struct V3dRegisterPcCtl {
	volatile unsigned int m_pctrc;
	volatile unsigned int m_pctre;
};
static struct V3dRegisterPcCtl __iomem *g_pV3dPcCtl =
		(struct V3dRegisterPcCtl __iomem *) IO_ADDRESS(0x20c00000 + 0x670);
...
static struct V3dRegisterDebugError __iomem *g_pV3dDebErr =
		(struct V3dRegisterDebugError __iomem *) IO_ADDRESS(0x20c00000 + 0xf00);

static void v3d_job_kill(v3d_job_t *p_v3d_job, v3d_job_status_e job_status);

static void StopThread(unsigned int t) {
	volatile unsigned int *pCs =
			t == 0 ? &g_pV3dCle->m_ct0cs : &g_pV3dCle->m_ct1cs;
	//put to stop mode
	*pCs = 1 << 5;
	barrier();

	KLOG_D("Thread %d stopped", t);
}

static void RunThread(unsigned int t, unsigned int ca, unsigned int ea) {
	volatile unsigned int *pCs =
			t == 0 ? &g_pV3dCle->m_ct0cs : &g_pV3dCle->m_ct1cs;
	volatile unsigned int *pCa =
			t == 0 ? &g_pV3dCle->m_ct0ca : &g_pV3dCle->m_ct1ca;
	volatile unsigned int *pEa =
			t == 0 ? &g_pV3dCle->m_ct0ea : &g_pV3dCle->m_ct1ea;
...
static unsigned int GetThreadReturnAddr(unsigned int t) {
	volatile unsigned int *pCa =
			t == 0 ? &g_pV3dCle->m_ct00ra0 : &g_pV3dCle->m_ct01ra0;
	return *pCa;
}
...
static int dbg_list_get_slot(v3d_job_post_t *p_job_post) {
	...
	if (size > DBG_MAX_LIST_SIZE) {
		KLOG_E("Skipping storing the CL for job[%d] size[%d]",
				p_job_post->job_id, size);
		return -1;
	}
...
static void dbg_list_store_job(v3d_job_post_t *p_job_post, int start,
		unsigned char *bin_addr, unsigned char *rend_addr) {
...
static int dbg_list_dump_all(void) {
	...
	dbg_filp = filp_open(fname, (O_WRONLY | O_TRUNC | O_LARGEFILE | O_CREAT),
			0666);
	if (dbg_filp == NULL) {
			...
			if (dbg_list_hdr[idx].job_post.job_id > dbg_last_job_written) {
				...
				if (dbg_filp != NULL) {
					if (dbg_filp->f_op && dbg_filp->f_op->write) {
						...
						dbg_filp->f_op->write(dbg_filp, addr, wr_sz,
								&dbg_filp->f_pos);

						addr = &dbg_data[dbg_list_hdr[idx].bin_start];
						wr_sz = dbg_list_hdr[idx].bin_size
						+ dbg_list_hdr[idx].rend_size;
						dbg_filp->f_op->write(dbg_filp, addr, wr_sz,
								&dbg_filp->f_pos);
					}
...
static int dbg_list(v3d_job_post_t *p_job_post, unsigned char *bin_addr,
		unsigned char *rend_addr) {
	int start;
...
static irqreturn_t v3d_isr(int irq, void *dev_id) {
	...
	if (v3d_flags != 0) {
		KLOG_E(
				"Assert - v3d_flags not handled, next interrupt has come v3d_flags[0x%x] v3d_flags[0x%x]",
				v3d_flags, (flags & 0x7));
	}
	v3d_flags = (flags & 0x3) | (flags_qpu ? (1 << 4) : 0);
//	KLOG_D("flags :0[%d] 1[%d] 2[%d] 3[%d] 4[%d] 5[%d]", flags & 0x1,
//			flags & 0x2, flags & 0x4, flags & 0x8, flags & 0x10, flags & 0x20);
//	KLOG_D("v3d_flags :0[%d] 1[%d] 2[%d] 3[%d] 4[%d] 5[%d]", v3d_flags & 0x1,
//			v3d_flags & 0x2, v3d_flags & 0x4, v3d_flags & 0x8, v3d_flags & 0x10,
//			v3d_flags & 0x20);
	...
	if (flags & (1 << 2)) {
		...
		if (v3d_oom_block_used == 0) {
...
#endif
			KLOG_D(
					"v3d oom blk not used: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
					flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
					v3d_read(BPCA), v3d_read(BPCS));
			iowrite32(v3d_bin_oom_block, v3d_base + BPOA);
			iowrite32(v3d_bin_oom_size, v3d_base + BPOS);
			v3d_oom_block_used = 1;
			KLOG_D(
					"v3d oom blk 1 given: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
					flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
					v3d_read(BPCA), v3d_read(BPCS));
		}
...
#endif
			KLOG_D(
					"v3d oom blk 1 used: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
					flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
					v3d_read(BPCA), v3d_read(BPCS));
			...
			KLOG_D(
					"v3d oom blk 2 given: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
					flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
					v3d_read(BPCA), v3d_read(BPCS));
		} else {
#ifdef DEBUG_V3D_ISR
...
#endif
			KLOG_D(
					"v3d fatal: oom blk 2 used: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
					flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
					v3d_read(BPCA), v3d_read(BPCS));
			v3d_flags |= (1 << 5);
		...
		KLOG_D(
				"Clear oom int: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
				flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
				v3d_read(BPCA), v3d_read(BPCS));
	}
	// Binner Used Overspill Memory interrupt status
	else if (flags & 0x08) {
		irq_retval = 1;
		KLOG_D(
				"v3d oom blk in use: flags[0x%02x] intctl[0x%08x] bpoa[0x%08x] bpos[0x%08x] bpca[0x%08x] bpcs[0x%08x]",
				flags, v3d_read(INTCTL), v3d_read(BPOA), v3d_read(BPOS),
				v3d_read(BPCA), v3d_read(BPCS));
		//Clear the flag
		iowrite32(0x8, v3d_base + INTCTL);
	}

	if (v3d_flags) {
		KLOG_D("JOB ENDED v3d_flags[0x%x]", v3d_flags);

		// Clear flags
		iowrite32(v3d_flags, v3d_base + INTCTL);
...
static void v3d_stat_update(struct timeval *p_tv) {
	...
	v3d_stat_time_spent_idx = (v3d_stat_time_spent_idx + 1)
			% V3D_STAT_WINDOW_SIZE;
	...
	v3d_stat_groups[v3d_stat_group_idx].avg_time_spent =
			(v3d_stat_groups[v3d_stat_group_idx].avg_time_spent
					+ v3d_stat_time_spent_acc) >> 1;
	v3d_stat_groups[v3d_stat_group_idx].min_time_spent = MIN(
			v3d_stat_groups[v3d_stat_group_idx].min_time_spent,
			v3d_stat_time_spent_acc);
	v3d_stat_groups[v3d_stat_group_idx].max_time_spent = MAX(
 ...
 */
static int v3d_proc_set_status(struct file *file, const char *buffer,
...
static void v3d_print_all_jobs(int bp) {
	...
	switch (bp) {
	case 0:
		KLOG_D("Job queue Status after post...");
		break;
	case 1:
		KLOG_D("Job queue Status after wait...");
		break;
	}
	if (bp > 1) {
...
static v3d_job_t *v3d_job_create(struct file *filp, v3d_job_post_t *p_job_post) {
	...
	KLOG_D(
			"job_create : job[0x%08x] dev_id[%d] job_id[%d] job_type[%d] ct0_ca[0x%x] ct0_ea[0x%x] ct1_ca[0x%x] ct1_ea[0x%x]",
			(u32 )p_v3d_job, dev->id, p_job_post->job_id, p_job_post->job_type,
			p_job_post->v3d_ct0ca, p_job_post->v3d_ct0ea, p_job_post->v3d_ct1ca,
			p_job_post->v3d_ct1ea);
...
static void v3d_job_add(struct file *filp, v3d_job_t *p_v3d_job, int pos) {
	...
	if (NULL == v3d_job_head) {
		KLOG_D("Adding job[0x%08x] to head[NULL]", (u32 )p_v3d_job);
		...
		KLOG_D("Adding job[0x%08x] to tail[0x%08x]", (u32 )p_v3d_job,
				(u32 )tmp_job);
...
static v3d_job_t *v3d_job_search(struct file *filp,
	...
	KLOG_D("Last job to wait for hdl[%d]: [0x%08x]", dev->id,
			(u32 )last_match_job);
...
static void v3d_job_free(struct file *filp, v3d_job_t *p_v3d_wait_job) {
	...
	KLOG_D("Free upto job[0x%08x] for hdl[%d]: ", (u32 )p_v3d_wait_job,
			dev->id);
		...
		while (tmp_job != NULL) {
			if (tmp_job->dev_id == dev->id) {
				...
				if (last_match_job == v3d_job_curr) {
					...
					KLOG_D("Trying to free current job[0x%08x]",
							(u32 )last_match_job);
				...
				KLOG_D("Free job[0x%08x] for hdl[%d]: ", (u32 )last_match_job,
						dev->id);
				if (p_v3d_wait_job == NULL) {
					KLOG_D("Free job[0x%08x] for hdl[%d]: ",
	...
	if (v3d_job_head != NULL) {
		if (v3d_job_head->dev_id == dev->id) {
			...
			if (last_match_job == v3d_job_curr) {
				/* Kill the job, free the job, return error if waiting ?? */
				KLOG_D("Trying to free current job - head[0x%08x]",
						(u32 )last_match_job);
			...
			KLOG_D("Update head to [0x%08x] and free [0x%08x] for hdl[%d]",
...
static int v3d_check_status(int state) {
	...
	if (((v3d_read(INTCTL) & 0x7) != 0) || (v3d_read(PCS) != 0)) {
		KLOG_E("state[%d] INTCTL[0x%08x] PCS[0x%08x]", state, v3d_read(INTCTL),
				v3d_read(PCS));
		print_status = 1;
	}
#ifdef DEBUG_V3D_ISR
	...
	if (dbg_v3d_prev_oom_blk2_cnt != dbg_v3d_oom_blk2_usage_cnt) {
		...
		KLOG_E(
				"state[%d] dbg_v3d_prev_oom_blk2_cnt[%d] dbg_v3d_oom_blk2_usage_cnt[%d]",
	...
	case 3: {
		if ((v3d_read(BFC) != 1) || (v3d_flags != 2)) {
			KLOG_E("state[%d] BFC[0x%08x] v3d_flags[0x%08x]", state,
					v3d_read(BFC), v3d_flags);
			print_status = 1;
		}
	}
		break;
	...
	case 4: {
		if ((v3d_read(RFC) != 1) || (v3d_flags != 1)) {
			KLOG_E("state[%d] RFC[0x%08x] v3d_flags[0x%08x]", state,
					v3d_read(RFC), v3d_flags);
			print_status = 1;
		}
#ifdef V3D_STATUS_PRINT_INTERVAL	
...
static void v3d_print_status(void) {
	KLOG_D(
			"v3d_print_status : v3d reg: ct0_ca[0x%x] ct0_ea[0x%x] ct1_ca[0x%x] ct1_ea[0x%x]",
			v3d_read(CT0CA), v3d_read(CT0EA), v3d_read(CT1CA), v3d_read(CT1EA));
	KLOG_D(
			"v3d_print_status : v3d reg: intctl[%x] pcs[%x] bfc[%d] rfc[%d] bpoa[0x%08x] bpos[0x%08x]",
			v3d_read(INTCTL), v3d_read(PCS), v3d_read(BFC), v3d_read(RFC),
			v3d_read(BPOA), v3d_read(BPOS));
	KLOG_D(
			"v3d_print_status : v3d reg: ct0cs[0x%08x] ct1cs[0x%08x] bpca[0x%08x] bpcs[0x%08x] \n",
			v3d_read(CT0CS), v3d_read(CT1CS), v3d_read(BPCA), v3d_read(BPCS));
#ifdef DEBUG_V3D_ISR
	KLOG_D(
			"v3d_print_status : Interrupt count[%d] bin[%d] rend[%d] qpu[%d] other[%d]",
			dbg_int_count, dbg_bin_int_count, dbg_rend_int_count,
			dbg_qpu_int_count, dbg_spurious_int_count);
	KLOG_D(
			"v3d_print_status : oom[%d] osm[%d] oom_blk1[%d] oom_blk2[%d] oom_fatal[%d]",
...
static void v3d_reg_init(void) {
	...
	iowrite32(0x8000 | (1 << 5), v3d_base + CT0CS);
	iowrite32(0x8000 | (1 << 5), v3d_base + CT1CS);
...
static int v3d_job_start(int turn_on) {
	...
	if (p_v3d_job->job_status != V3D_JOB_STATUS_READY) {
		if ((p_v3d_job->job_status != V3D_JOB_STATUS_RUNNING)
			...
			KLOG_E(
					...
					(u32 )p_v3d_job, p_v3d_job->job_status, p_v3d_job->job_type,
	...
	if ((p_v3d_job->job_type == V3D_JOB_REND)
			&& (p_v3d_job->job_intern_state == 0)) {
		KLOG_D("V3D_JOB_REND RENDERER launching, job_intern_state==0");
		...
		KLOG_D("CT1CA : 0x%08x", p_v3d_job->v3d_ct1ca);
		KLOG_D("CT1EA : 0x%08x", p_v3d_job->v3d_ct1ea);

		RunThread(1, p_v3d_job->v3d_ct1ca, p_v3d_job->v3d_ct1ea);

	} else if ((p_v3d_job->job_type == V3D_JOB_BIN_REND)
			&& (p_v3d_job->job_intern_state == 0)) {
		KLOG_D("V3D_JOB_BIN_REND BINNER launching...");
		...
		KLOG_D("CTOCA : 0x%08x", p_v3d_job->v3d_ct0ca);
		KLOG_D("CTOEA : 0x%08x", p_v3d_job->v3d_ct0ea);

		RunThread(0, p_v3d_job->v3d_ct0ca, p_v3d_job->v3d_ct0ea);

	} else if ((p_v3d_job->job_type == V3D_JOB_BIN_REND)
			&& (p_v3d_job->job_intern_state == 1)) {
		KLOG_D("V3D_JOB_BIN_REND BINNER launching, job_intern_state==1");
		p_v3d_job->job_intern_state = 2;
		v3d_reg_clr_cache();
...
static void v3d_job_kill(v3d_job_t *p_v3d_job, v3d_job_status_e job_status) {
	KLOG_D("Kill job[0x%08x]: ", (u32 )p_v3d_job);
...
static void v3d_job_reset(v3d_job_t *p_v3d_job) {
	KLOG_D("Reset job[0x%08x]: ", (u32 )p_v3d_job);
...
static int v3d_thread(void *data) {
	...
	while (1) {
		KLOG_D("v3d_job_curr [%p]", v3d_job_curr);
		if (v3d_job_curr == NULL) {
			/* No jobs pending - wait till a job gets posted */
			KLOG_D("v3d_thread going to sleep till a post happens");
			while (v3d_job_curr == NULL) {
//				mutex_unlock(&v3d_mutex);
				up(&v3d_sem);
				if (wait_event_interruptible(v3d_start_q,
						(v3d_job_curr != NULL))) {
			...
			KLOG_D("Signal received to launch job");
			ret = v3d_job_start(1);
		} else {
...
//			mutex_unlock(&v3d_mutex);
			...
			timeout = wait_event_interruptible_timeout(v3d_isr_done_q,
					(v3d_in_use == 0), msecs_to_jiffies(V3D_ISR_TIMEOUT_IN_MS));
			KLOG_D("wait exit, v3d_in_use[%d], timeout[%d]", v3d_in_use,
					timeout);
			if (timeout && (timeout < timout_min)) {
				...
				KLOG_D(
						...
						timout_min,
						(u32)msecs_to_jiffies(V3D_ISR_TIMEOUT_IN_MS));
			}

			if ((timeout != 0) && (v3d_in_use != 0)) {
				KLOG_E("wait interrupted, v3d_in_use[%d], timeout[%d]",
						v3d_in_use, timeout);
				do_exit(-1);
			}
			if (down_interruptible(&v3d_sem)) {
			...
			if (v3d_in_use == 0) {
				...
				if ((v3d_job_curr->job_type == V3D_JOB_BIN_REND)
				...
				} else if (((v3d_job_curr->job_type == V3D_JOB_BIN_REND)
						&& (v3d_job_curr->job_intern_state == 2))
						|| ((v3d_job_curr->job_type == V3D_JOB_REND)
								&& (v3d_job_curr->job_intern_state == 2))) {
				...
				if (v3d_job_curr->cache_retry_cnt <= V3D_CACHE_MAX_RETRIES) {
					KLOG_D(
							"Cache Timeout[%d]ms Retry[%d] for job[0x%08x] retry[%d]",
							V3D_ISR_TIMEOUT_IN_MS,
							v3d_job_curr->cache_retry_cnt,
							(unsigned int )v3d_job_curr,
							v3d_job_curr->retry_cnt);
					KLOG_D(
							"v3d reg: ct0_ca[0x%x] ct0_ea[0x%x] ct1_ca[0x%x] ct1_ea[0x%x]",
							v3d_read(CT0CA), v3d_read(CT0EA), v3d_read(CT1CA),
							v3d_read(CT1EA));
					KLOG_D(
							"v3d reg: intctl[%x] pcs[%x] bfc[%d] rfc[%d] bpoa[0x%08x] bpos[0x%08x]",
							v3d_read(INTCTL), v3d_read(PCS), v3d_read(BFC),
							v3d_read(RFC), v3d_read(BPOA), v3d_read(BPOS));
					KLOG_D(
							"v3d reg: ct0cs[0x%08x] ct1cs[0x%08x] bpca[0x%08x] bpcs[0x%08x] \n",
							v3d_read(CT0CS), v3d_read(CT1CS), v3d_read(BPCA),
							v3d_read(BPCS));
					continue;
				}
#endif
...
#if 1
				KLOG_D(
						"job[0x%08x] dev_id[%d] job_id[%d] type[%d] status[%d] intern[%d] wait[%d] retry[%d][%d]",
						(u32 )v3d_job_curr, v3d_job_curr->dev_id,
						v3d_job_curr->job_id, v3d_job_curr->job_type,
						v3d_job_curr->job_status,
						v3d_job_curr->job_intern_state,
						v3d_job_curr->job_wait_state,
						v3d_job_curr->cache_retry_cnt, v3d_job_curr->retry_cnt);
				KLOG_D(
						"job: ct0_ca[0x%x] ct0_ea[0x%x] ct1_ca[0x%x] ct1_ea[0x%x]",
						v3d_job_curr->v3d_ct0ca, v3d_job_curr->v3d_ct0ea,
						v3d_job_curr->v3d_ct1ca, v3d_job_curr->v3d_ct1ea);
				v3d_print_status();
#endif				
#ifdef V3D_JOB_RETRY_ON_TIMEOUT
...
static int v3d_job_post(struct file *filp, v3d_job_post_t *p_job_post) {
	...
	printk(
	KERN_INFO "POST JOB type %d id %d %08x/%08x %08x/%08x size %d\n",
			p_job_post->job_type, p_job_post->job_id, p_job_post->v3d_ct0ca,
			p_job_post->v3d_ct0ea, p_job_post->v3d_ct1ca, p_job_post->v3d_ct1ea,
			p_job_post->v3d_vpm_size);

	dev = (v3d_t *) (filp->private_data);
	if ((p_job_post->job_type != V3D_JOB_REND)
			&& (p_job_post->job_type != V3D_JOB_BIN_REND)) {
...
//	if (mutex_lock_interruptible(&v3d_mutex)) {
//		KLOG_E("lock acquire failed");
//		ret = -ERESTARTSYS;
//		goto err;
//	}
	...
	if (NULL == v3d_job_curr) {
		KLOG_D("No job pending : Signal to v3d thread about post");
...
//				(u32 )p_v3d_wait_job, p_v3d_wait_job->dev_id);
		...
		while ((p_v3d_wait_job->job_status == V3D_JOB_STATUS_READY)
			...
			up(&v3d_sem);
//			mutex_unlock(&v3d_mutex);
			if (wait_event_interruptible(p_v3d_wait_job->v3d_job_done_q,
					((p_v3d_wait_job->job_status != V3D_JOB_STATUS_READY)
...
//		KLOG_D("Wait ioctl to return status[%d] for job[0x%08x]",
...
//	}
	...
	dev->id = v3d_id++;
	KLOG_D("in open for id[%d]", dev->id);
	up(&v3d_sem);
//	mutex_unlock(&v3d_mutex);
	...
	KLOG_D("v3d id[%d]: pool phys[0x%08x] virt[0x%08x] size[0x%08x]", dev->id,
			v3d_mempool_phys_base, (int )v3d_mempool_base,
			(int )v3d_mempool_size);
...
//	}
	...
	KLOG_D("after free for id[%d]", dev->id);
	v3d_print_all_jobs(0);
...
static int v3d_mmap(struct file *filp, struct vm_area_struct *vma) {
	...
	dev = (v3d_t *) (filp->private_data);
//	KLOG_D("mmap: id[%d] start[0x%08x], size[0x%08x] pgoff[0x%08x]", dev->id,
//			(u32 )vma->vm_start, (int )vma_size, (u32 )vma->vm_pgoff);
	...
	if (!vma->vm_pgoff) {
		...
		KLOG_E("mmap vm_pgoff[%d] should be v3d pool base or 0.",
				(u32 )vma->vm_pgoff);
		return -EINVAL;
	}
...
static v3d_job_status_t jobStatus;

static int v3d_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	v3d_t *dev;
	int ret = 0;
	...
	case V3D_IOCTL_POST_JOB: {
		v3d_job_post_t job_post;
		if (copy_from_user(&job_post, (v3d_job_post_t *) arg,
				sizeof(job_post))) {
	...
	case V3D_IOCTL_WAIT_JOB: {
		...
		if (copy_to_user((v3d_job_status_t *) arg, &job_status,
				sizeof(job_status))) {
	...
	case V3D_IOCTL_GET_MEMPOOL: {
		...
		mempool.ptr = v3d_mempool_base; //phys_to_virt(v3d_mempool_base);
...
static struct file_operations v3d_fops = { .open = v3d_open, .release =
		v3d_release, .mmap = v3d_mmap, .unlocked_ioctl = v3d_ioctl, };
...
struct platform_driver v3d_driver = { .driver = { .name = "v3d_dev", .owner =
THIS_MODULE, }, .suspend = v3d_suspend, .resume = v3d_resume, };
...
int __init v3d_opt_init(void) {
	...
	v3d_base = (void __iomem *) IO_ADDRESS(V3D_BASE);

	KLOG_D("v3d_base phys [0x%08x]", v3d_base);
...
//check it's come up
	...
	v3d_bin_oom_cpuaddr = dma_alloc_coherent(NULL, v3d_bin_oom_size,
			&v3d_bin_oom_block, GFP_ATOMIC | GFP_DMA);
	...
	KLOG_D("IRQ_GRAPHICS Request...");
	KLOG_D("v3d_dev [%p]", v3d_dev);
	irq_err = request_irq(/*IRQ_GRAPHICS*/INTERRUPT_3D, v3d_isr,
	IRQF_DISABLED | IRQF_SHARED, V3D_DEV_NAME, v3d_dev);

	if (irq_err) {
		KLOG_D("Registering isr failed");
		goto err;
	}
	KLOG_D("IRQ_GRAPHICS Requested");
	...
	if (v3d_bin_oom_cpuaddr2)
		dma_free_coherent(NULL, v3d_bin_oom_size2, v3d_bin_oom_cpuaddr2,
				v3d_bin_oom_block2);
	if (v3d_bin_oom_cpuaddr)
		dma_free_coherent(NULL, v3d_bin_oom_size, v3d_bin_oom_cpuaddr,
				v3d_bin_oom_block);
	if (v3d_dev)
		kfree(v3d_dev);
	if (v3d_base)
