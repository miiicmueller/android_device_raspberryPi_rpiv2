...
unsigned int AllocateVcMemory(unsigned int *pHandle, unsigned int size,
...
//run the command
	...
	if (s == 0 && msg.m_response == 0x80000000
			&& msg.m_tag.m_recvDataSize == 0x80000004) {
		*pHandle = msg.m_tag.m_args.m_handle;
		return 0;
	} else {
		printk(
				KERN_ERR "failed to allocate vc memory: s=%d response=%08x recv data size=%08x\n",
...
//pass across the handle
	...
	if (s == 0 && msg.m_response == 0x80000000
			&& msg.m_tag.m_recvDataSize == 0x80000004
			&& msg.m_tag.m_args.m_error == 0)
		return 0;
	else {
		printk(
				KERN_ERR "failed to release vc memory: s=%d response=%08x recv data size=%08x error=%08x\n",
				s, msg.m_response, msg.m_tag.m_recvDataSize,
				msg.m_tag.m_args.m_error);
		return 1;
	}
}
...
//check the error code too
	if (s == 0 && msg.m_response == 0x80000000
		...
		printk(
				KERN_ERR "failed to unlock vc memory: s=%d response=%08x recv data size=%08x error%08x\n",
				s, msg.m_response, msg.m_tag.m_recvDataSize,
				msg.m_tag.m_args.m_error);
		return 1;
	}
...
unsigned int ExecuteVcCode(unsigned int code, unsigned int r0, unsigned int r1,
		unsigned int r2, unsigned int r3, unsigned int r4, unsigned int r5) {
	struct vc_msg {
		unsigned int m_msgSize;
		unsigned int m_response;
...
//check the error code too
	if (s == 0 && msg.m_response == 0x80000000
		...
		printk(
				KERN_ERR "failed to execute: s=%d response=%08x recv data size=%08x\n",
				s, msg.m_response, msg.m_tag.m_recvDataSize);
		return 1;
	}
}
